...
```{r}
Region_multreg <- lm(LeavePct ~ Reform_share + Lab_share + LD_share + Green_share + Con_share, data = Yorkshire_results)
...
```{r}
Reform_multreg <- lm(LeavePct ~ Reform_share + Con_share + Green_share + LD_share + Lab_share + PC_share, data = ENG_WLS_results)
tidy(Region_multreg) |>
     knitr::kable(digits = 3)
```
...
York_multreg <- lm(Reform_share ~ LeavePct, data = Yorkshire_results)
